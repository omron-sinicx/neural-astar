"""SAIL planners (computing cdist on GPU).
Author: Ryo Yonetani
...
class SAILGPU(nn.Module):
    """
    Implementation of the SAIL planner 
    """
    def __init__(self, mechanism, beta0, num_skips, Tmax=1., Nmax=60, cdist_factor=50, verbose=False):
        super().__init__()
        ...
        self.cdist_factor = cdist_factor 
    ...
    def forward(self, map_designs, start_maps, goal_maps, opt_dists):
        ...
        cf = self.cdist_factor
        for i in [[0, 1], [1, 2], [0, 2]]:
            d2o_val, d2o_loc = [], []
            # break cdist computation into several chunks to make it memory-efficient
            for t in range(int(len(pop) / cf)):
                _cdist = torch.cdist(pop[(cf * t):(cf * (t + 1)), :, i[0]:i[1]], oep[(cf * t):(cf * (t + 1)), :, i[0]:i[1]])
                _d2o_val, _d2o_loc = _cdist.min(dim=2)
                d2o_val.append(_d2o_val)
                d2o_loc.append(_d2o_loc)
            d2o_val = torch.cat(d2o_val)
            d2o_loc = torch.cat(d2o_loc)
        ...
        for t in range(Tmax):
            ...
            if np.mod(t, self.num_skips) == 0:
                ...
                sail_feat.append((feat * oh.unsqueeze(-1)).sum((1, 2)))
                sail_oracle_q.append((opt_dists * oh).sum((1, 2)))
        ...
        if(len(sail_feat) > Nmax):
            sail_feat = sail_feat[0:-1:int(len(sail_feat)/Nmax)][:Nmax]
            sail_oracle_q = sail_oracle_q[0:-1:int(len(sail_oracle_q)/Nmax)][:Nmax]
        if self.training:
            qnet_out = self.qnet(torch.cat(sail_feat))
        else:
            with torch.no_grad():
                qnet_out = self.qnet(torch.cat(sail_feat))

        sail_oracle_q = torch.cat(sail_oracle_q).unsqueeze(1)
        training_pairs = [qnet_out, sail_oracle_q]

        return histories.unsqueeze(1), path_maps.unsqueeze(1), training_pairs


@gin.configurable
class SAILGPUPlanner(_ILPlanner):
    def __init__(self, mechanism, loss_fn, sample_from, skip_exp_when_training=True, model_cls=SAILGPU):
